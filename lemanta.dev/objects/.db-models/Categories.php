<?php
    ...
    */
    ...
    class CategoriesDBModel extends BasicDBModel {
        ...
        public $dbtable = 'categories';
        public $id_field = 'category_id';
                    ...
                    if (!empty($item)) {
                        ...
                        if (!isset($item->products_count)) {
                                ...
                                if (isset($item->products)) {
                                    ...
                                    if (isset($item->category_id)) {
                                        ...
                                        $query = 'SELECT COUNT(`enabled`) AS `count` '
                                               . 'FROM `products` '
                                               . 'WHERE `' . $this->id_field . '` = "' . $this->cms->db->query_value($item->category_id) . '" '
                            ...
                            if (!empty($item->subcategories)) {
                                $count += $this->compute_products($item->subcategories, $params);
                            }
            ...
            if (!empty($items)) {
                foreach ($items as & $item) {
                    if (!empty($item)) {
                        ...
                        // если у ветки есть вложенные ветки, делаем рекурсивный вызов и приплюсовываем к итогу
                        if (!empty($item->subcategories)) {
                            $total += $this->compute($item->subcategories);
                        }
                    }
                }
            }
        ...
        public function get ( & $items, $params = null, & $items_list = null ) {
            $this->cms->db->open_tracing_method('DB ' . strtoupper($this->dbtable) . ' get');
                ...
                if (isset($params->sort)) {
                    switch ($params->sort) {
                        case SORT_CATEGORIES_MODE_BY_NAME:
                            $order = '`' . $this->dbtable . '`.`name` ASC, '
                                   . '`' . $this->dbtable . '`.`order_num` DESC ';
                            break;
                        case SORT_CATEGORIES_MODE_BY_BROWSED:
                            $order = '`' . $this->dbtable . '`.`browsed` DESC, '
                                   . '`' . $this->dbtable . '`.`order_num` DESC ';
                            break;
                        case SORT_CATEGORIES_MODE_BY_URL:
                            $order = '`' . $this->dbtable . '`.`url` ASC, '
                                   . '`' . $this->dbtable . '`.`order_num` DESC ';
                            break;
                        case SORT_CATEGORIES_MODE_BY_MENU:
                            $order = '`menu` DESC, '
                                   . '`' . $this->dbtable . '`.`order_num` DESC ';
                            break;
                        case SORT_CATEGORIES_MODE_BY_OBJECTS:
                            $order = '`' . $this->dbtable . '`.`objects` DESC, '
                                   . '`' . $this->dbtable . '`.`order_num` DESC ';
                            break;
                        case SORT_CATEGORIES_MODE_AS_IS:
                        default:
                ...
                $select = '`' . $this->dbtable . '`.*, '
                 ...
                 . 'LEFT JOIN `products_categories` '
                           . 'ON `products_categories`.`' . $this->id_field . '` = `' . $this->dbtable . '`.`' . $this->id_field . '` '
                ...
                if (!$state) {
                    $this->cms->db->open_tracing_method('DB ' . strtoupper($this->dbtable) . ' get [checkQuery]');
                        ...
                        if ($this->cms->db->caching_SELECT($select,
                                                           ...
                                                           DATABASE_CACHE_CATEGORIES_LIFETIME,
...
// TODO: change this later
                                    $this->cms->db->fix_categories_record($item);
                                    $items_list[$item->category_id] = $item;
                                }
                            }
...
// TODO: change this later
                ...
                // строим в переменной $items дерево категорий
                                ...
                                if ($parent != $id) {
                                    if ($parent == 0) {
                                        if (!isset($used[$id])) {
                                            ...
                                            $path->category_id = $id;
                                            $path->url = $item->url;
                                            $path->url_path = isset($item->url_special) && $item->url_special ? '' : 'catalog/';
                                            $item->path[0] = $path;
                                            $items[$id] = & $item;
                                            $used[$id] = & $items[$id];
                                        ...
                                        if (isset($used[$parent])) {
                                            if (!isset($used[$parent]->subcategories[$id])
                                            && !$this->in_branch($item, $parent)) {
                                                ...
                                                $item->path[] = $path;
                                                $used[$parent]->subcategories[$id] = & $item;
                                                $stop_via_trash = FALSE;
                                                if (!isset($used[$id])) $used[$id] = & $used[$parent]->subcategories[$id];
                                            }
                                        } else {
                ...
                // заполняем в элементах массивы subcats_ids (идентификаторы подкатегорий)
                $this->cms->db->open_tracing_method('DB ' . strtoupper($this->dbtable) . ' get [fillSubcatsIds]');
                    $used = array_reverse($used, TRUE);
                    foreach ($used as & $item) {
                        $used[$item->category_id]->subcats_ids[] = $item->category_id;
                        foreach ($item->parents as $parent) {
                            if ($parent != 0) {
                ...
                $this->cms->db->open_tracing_method('DB ' . strtoupper($this->dbtable) . ' get [computing]');
                    ...
                    // подсчитываем количество товаров в ветвях категорий
                    $params->enabled = 1;
                    $this->compute_products($items, $params);

                    // подсчитываем количество прочитанных категорий
                    $count = $this->compute($items);

                $this->cms->db->close_tracing_method();

            // возвращаем количество прочитанных категорий
            $this->cms->db->close_tracing_method();
            return $count;
        }
        ...
        // =======================================================================
        // Обновить/добавить запись о категории в базе данных:
        ...
        public function update ( & $item ) {
            $this->cms->db->open_tracing_method('DB CATEGORIES update');
            ...
            if (!empty($item)) {
                ...
                if (isset($item->own_block))         {$fields[] = 'own_block';         $values[] = '"' . $this->cms->db->query_value($this->cms->db->value_as_boolean($item->own_block)) . '"';}
                ...
                if (isset($item->single_name))       {$fields[] = 'single_name';       $values[] = '"' . $this->cms->db->query_value($this->cms->db->value_as_string($item->single_name)) . '"';}
                if (isset($item->configurator_name)) {$fields[] = 'configurator_name'; $values[] = '"' . $this->cms->db->query_value($this->cms->db->value_as_string($item->configurator_name)) . '"';}
                ...
                $id = $this->cms->db->update_record($item, 'categories', 'category_id', $fields, $values);

                // если выполнено
                if (!empty($id)) {

                    // проверяем необходимость очистить кеш-таблицы категорий
                    $this->cms->db->reset_categories_caches($item);
                }
            }
